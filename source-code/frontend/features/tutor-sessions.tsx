...
import {
  ...
  getTutorCitationContext, getTutorPractice, getTutorPracticeHint, getTutorSessions, getNextTutorTopic,
...
export function TutorSessions({ notify }: { notify: (message: string) => void }) {
  ...
  const [subject, setSubject] = useState(''), [topic, setTopic] = useState(''), [profile, setProfile] = useState('');
  ...
  const [nextTopic, setNextTopic] = useState<NextTopicResult | null>(null);
  ...
  useEffect(() => { if (subjectChoice && !subjectChoice.topics.some((item) => item.topicRef === topic)) setTopic(subjectChoice.topics[0]?.topicRef || ''); }, [subjectChoice, topic]);
  ...
  const act = async (operation: () => Promise<TutorSession>, confirmation?: string) => {
    ...
    try { const result = await operation(); setSession(result.status === 'active' ? result : null); setNextTopic(null); setSources(null); setExpandedSource(null); setAgentReply(null); if (confirmation) notify(confirmation); }
    catch (cause) { setError(errorMessage(cause)); }
  ...
  if (!session) return <div className="stack"><Heading eyebrow="STUDENT · PRACTICE" title="Start a tutor session">Choose one covered topic and one of your tutors. Tutor help is available during practice only.</Heading>
    ...
    {!options.profiles.length ? <Empty title="Create a tutor first">Go to My tutors and build an AKURU tutor profile.</Empty> : !options.subjects.length ? <Empty title="No covered topics available">Ask your parent or administrator to check your enrolment and curriculum coverage.</Empty> : <form className="panel stack tutor-session-start" onSubmit={(event) => { event.preventDefault(); void act(() => startTutorSession(subject, topic, profile, key()), 'Tutor session started.'); }}>
      ...
      <label className="stack"><span>Covered topic</span><select value={topic} onChange={(event) => setTopic(event.target.value)}>{subjectChoice?.topics.map((item) => <option key={item.topicRef} value={item.topicRef}>{item.groupCode} · {item.code} · {item.title}</option>)}</select></label>
      ...
      <Button className="primary" type="submit" disabled={busy || !topic || !profile}><MessageCircle size={16} />{busy ? 'Starting…' : 'Enter tutor room'}</Button>
    </form>}
  </div>;
  const topics = options.subjects.find((item) => item.id === session.subjectId)?.topics || [];
  const activeLabel = `${session.activeTopic.groupCode} · ${session.activeTopic.code} · ${session.activeTopic.title}`;
  return <div className="stack"><Heading eyebrow="STUDENT · TUTOR ROOM" title={`${session.currentTutor.name} is helping you`} action={<Button variant="outline" disabled={busy} onClick={() => void act(() => endTutorSession(session.sessionRef, key()), 'Tutor session ended.')}><Square size={14} />End session</Button>}>Your transcript stays together when you change tutors or move to another covered topic.</Heading>
    {error && <div className="error" role="alert">{error}</div>}
    {quota && <QuotaSummary quota={quota} />}
    <section className="panel tutor-room-header">
      ...
      <div className="tutor-room-controls">
        ...
        <label><span>Move topic</span><select value={session.activeTopic.topicRef} disabled={busy} onChange={(event) => void act(() => switchTutorTopic(session.sessionRef, event.target.value, key()), 'Active topic changed.')}>{topics.map((item) => <option key={item.topicRef} value={item.topicRef}>{item.groupCode} · {item.code} · {item.title}</option>)}</select></label>
      </div>
    </section>
    <section className="panel stack tutor-next-topic">
      <div className="panel-heading"><div><span className="pill"><Compass size={13} /> EVIDENCE-BASED</span><h2>What should I improve next?</h2></div><Button variant="outline" disabled={busy} onClick={async () => { setBusy(true); setError(''); try { setNextTopic(await getNextTutorTopic(session.sessionRef, session.subjectId, key())); } catch (cause) { setError(errorMessage(cause)); } finally { setBusy(false); } }}><Compass size={15} />Find next topic</Button></div>
      {nextTopic && (nextTopic.status !== 'ready' || !nextTopic.recommendation ? <p>{nextTopic.message}</p> : <article className="card stack"><div><span className="pill">PRIORITY TOPIC</span><h3>{`${nextTopic.recommendation.topic.groupCode} · ${nextTopic.recommendation.topic.code} · ${nextTopic.recommendation.topic.title}`}</h3></div><p>{nextTopic.recommendation.reason}</p><div><strong>{nextTopic.recommendation.activity.title}</strong><p>{nextTopic.recommendation.activity.instruction}</p><small>Success: {nextTopic.recommendation.activity.successCondition}</small></div><Button className="primary" disabled={busy || nextTopic.recommendation.topic.topicRef === session.activeTopic.topicRef} onClick={() => void act(() => switchTutorTopic(session.sessionRef, nextTopic.recommendation!.topic.topicRef, key()), 'Moved to the recommended topic.')}><ArrowRightLeft size={15} />Move to this topic</Button></article>)}
    </section>
