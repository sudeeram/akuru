...
Defaults are: submitted answers/result evidence 730 days, handwriting files 90 days, rejected or never-published generated media 30 days, and security audit records 2,190 days. `python -m app.retention` is a dry run; add `--apply` to redact expired answers and generated feedback and delete expired private objects. `--student-id UUID --apply` ignores age thresholds and immediately redacts that child's answer/feedback artifacts and deletes all of their working files after an authorized request. It retains minimum assessment scores and audit history for learning-integrity records. The daily systemd timer applies the policy. Backups expire separately after 30 days, so deleted content ages out of recovery media within that window.
