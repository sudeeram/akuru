...
from app.models import (
    ...
    StudyPlanItem, TextbookGroup, TextbookTopic, TextbookUnit, TopicMastery, TopicMasteryDimension,
    TopicMasteryEvent, TutorLearnerContextLog, TutorSession, UnitMastery, UnitMasteryDimension,
    UnitMasteryEvent, WeaknessDiagnosis,
)
...
from app.schemas.tutor_context import (
    ...
    ContextReviewedRecommendation, ContextStatement, ContextTopic, ContextUnit, LearnerContextResponse,
...
def _build(db: Session, student_id: uuid.UUID, session: TutorSession, operation_ref: str, generated_at: datetime):
    if session.active_topic_id:
        return _build_topics(db, student_id, session, operation_ref, generated_at)
...
def _build_topics(db: Session, student_id: uuid.UUID, session: TutorSession, operation_ref: str, generated_at: datetime):
    coverage=curriculum_plans.student_coverage(db,student_id,session.subject_id)
    ...
    rows=db.execute(select(TextbookTopic,TextbookGroup).join(TextbookGroup,TextbookGroup.id==TextbookTopic.group_id
        ).where(TextbookTopic.public_ref.in_(refs)).order_by(TextbookGroup.sequence,TextbookTopic.sequence)).all()
    topics=[]; evidence=[]
    for topic,group in rows:
        mastery=db.scalar(select(TopicMastery).where(TopicMastery.student_id==student_id,TopicMastery.topic_id==topic.id))
        events=db.scalars(select(TopicMasteryEvent).join(AssessmentResult,AssessmentResult.id==TopicMasteryEvent.trigger_result_id
        ...
        label=f"{group.code} · {topic.code} – {topic.title}"; statements=[]; signals=[]
        if not mastery or not events:
            signals=["insufficient_evidence"]; statements=[ContextStatement(text=f"I do not yet have assessed evidence about {label}.",
                signal="insufficient_evidence",evidenceRefs=[],cautious=True)]
        else:
            score=float(mastery.display_score); refs2=[_reference("topic_mastery",event.id) for event in events]
            signal="strong" if score>=7 else "weak" if score<=4 else "assessed"
            signals=[signal] if signal!="assessed" else []
            statements=[ContextStatement(text=f"Your assessed topic mastery for {label} is {score:.1f}/10.",
                signal=signal if signal!="assessed" else "low_confidence",evidenceRefs=refs2,
                cautious=mastery.confidence=="low")]
            evidence.extend(ContextEvidence(ref=ref,kind="mastery_event",observedAt=event.created_at,
                ...
                for ref,event in zip(refs2,events))
        attempts=[]
        for result,question,assessment in db.execute(select(AssessmentResult,AssessmentQuestion,Assessment).join(
            AssessmentQuestion,AssessmentQuestion.id==AssessmentResult.question_id).join(Assessment,Assessment.id==AssessmentResult.assessment_id
            ).where(Assessment.student_id==student_id,Assessment.subject_id==session.subject_id,
            AssessmentResult.status=="published").order_by(AssessmentResult.created_at.desc())).all():
            if str(topic.id) not in (question.topic_ids or []): continue
            ref=_reference("assessment",result.id,result.version_number); missing=[str(x.get("rationale",""))[:240] for x in result.marking_decisions if not x.get("awarded")]
            attempts.append(ContextAttempt(evidenceRef=ref,observedAt=result.created_at,mode=assessment.mode,
                score=round(result.awarded_marks/result.max_marks*10,1),awardedMarks=result.awarded_marks,
                maxMarks=result.max_marks,markingSummary=missing[:8]))
            evidence.append(ContextEvidence(ref=ref,kind="assessment_result",observedAt=result.created_at,
        ...
        mistakes=[]
        for diagnosis in db.scalars(select(WeaknessDiagnosis).where(WeaknessDiagnosis.student_id==student_id,
            WeaknessDiagnosis.topic_id==topic.id).order_by(WeaknessDiagnosis.created_at)).all():
            ref=_reference("mistake",diagnosis.id); statement=ContextStatement(text=f"AKURU found {diagnosis.category.replace('_',' ')} in {topic.title}.",
                signal="recurring_mistake",evidenceRefs=[ref],cautious=diagnosis.confidence<.75)
            mistakes.append(ContextMistakePattern(topic=topic.title,category=diagnosis.category,last7Days=int(diagnosis.created_at>=generated_at-timedelta(days=7)),
                last30Days=int(diagnosis.created_at>=generated_at-timedelta(days=30)),lifetime=diagnosis.occurrence_number,evidenceRefs=[ref],statement=statement))
            evidence.append(ContextEvidence(ref=ref,kind="mistake",observedAt=diagnosis.created_at,summary=diagnosis.description))
        recommendations,recommendation_evidence=_reviewed_topic_recommendations(db,student_id,session.subject_id,topic.id)
        plans,plan_evidence=_topic_plan_items(db,student_id,session.subject_id,topic.id); evidence.extend(recommendation_evidence+plan_evidence)
        dimensions={row.dimension:float(row.score) for row in db.scalars(select(TopicMasteryDimension).where(
            TopicMasteryDimension.mastery_id==mastery.id)).all()} if mastery and events else {}
        topics.append(ContextTopic(topic=PlanTopicResponse(topicRef=topic.public_ref,code=topic.code,title=topic.title,
            groupRef=group.public_ref,groupCode=group.code,groupTitle=group.title),active=topic.id==session.active_topic_id,
            masteryScore=float(mastery.display_score) if mastery and events else None,confidence=mastery.confidence if mastery and events else None,
            trend=float(mastery.trend) if mastery and events else None,evidenceCount=len(events),varietyCount=mastery.variety_count if mastery and events else 0,
            dimensions=dimensions,signals=signals,statements=statements,attempts=attempts[:10],mistakes=mistakes,
            reviewedRecommendations=recommendations,studyPlan=plans))
    evidence=list({row.ref:row for row in evidence}.values()); evidence.sort(key=lambda row:row.ref)
    digest=hashlib.sha256("|".join([CONTEXT_ALGORITHM,session.subject_id,active.public_ref]+[x.ref for x in evidence]).encode()).hexdigest()
    active_response=next(row.topic for row in topics if row.active)
    provider=ProviderLearnerContext(learnerRef="learner_"+hashlib.sha256(f"akuru:{student_id}".encode()).hexdigest()[:20],
        subjectId=session.subject_id,activeTopicCode=active.code,contextVersion=digest,
        learningSignals=[{"topicRef":x.topic.topicRef,"group":x.topic.groupTitle,"signals":x.signals,
            "statements":[s.model_dump(mode="json") for s in x.statements]} for x in topics],
        recurringMistakes=[{"topicRef":x.topic.topicRef,"topic":m.topic,"last7Days":m.last7Days,
            "evidenceRefs":m.evidenceRefs} for x in topics for m in x.mistakes],
        reviewedRecommendations=[{"topicRef":x.topic.topicRef,"title":r.title,"activityType":r.activityType,
            "evidenceRef":r.evidenceRef} for x in topics for r in x.reviewedRecommendations],
        plannedActivities=[{"topicRef":x.topic.topicRef,"title":p.title,"activityType":p.activityType,
            "evidenceRef":p.evidenceRef} for x in topics for p in x.studyPlan])
    return LearnerContextResponse(operationRef=operation_ref,contextVersion=digest,generatedAt=generated_at,
        subjectId=session.subject_id,activeTopic=active_response,topics=topics,evidence=evidence,providerContext=provider)

def _reviewed_topic_recommendations(db, student_id, subject_id, topic_id):
    rows=db.scalars(select(ImprovementRecommendation).where(ImprovementRecommendation.student_id==student_id,
        ImprovementRecommendation.subject_id==subject_id,ImprovementRecommendation.topic_id==topic_id,
        ImprovementRecommendation.review_status=="approved").order_by(ImprovementRecommendation.created_at)).all()
    return ([ContextReviewedRecommendation(evidenceRef=_reference("recommendation",row.id),title=row.title,
        activityType=row.activity_type,reason=row.reason,action=row.action,successCondition=row.success_condition) for row in rows],
        [ContextEvidence(ref=_reference("recommendation",row.id),kind="reviewed_recommendation",observedAt=row.created_at,
        summary=row.title) for row in rows])

def _topic_plan_items(db, student_id, subject_id, topic_id):
    rows=db.scalars(select(StudyPlanItem).join(StudyPlan,StudyPlan.id==StudyPlanItem.plan_id).where(
        StudyPlan.student_id==student_id,StudyPlan.status=="active",StudyPlanItem.subject_id==subject_id,
        StudyPlanItem.topic_id==topic_id).order_by(StudyPlanItem.sequence)).all()
    return ([ContextPlanItem(evidenceRef=_reference("plan",row.id),title=row.title,activityType=row.activity_type,
        status=row.status,scheduledFor=row.scheduled_for) for row in rows],
        [ContextEvidence(ref=_reference("plan",row.id),kind="study_plan_item",observedAt=row.created_at,summary=row.title) for row in rows])
...
def build_and_log(db: Session, settings: Settings, principal: Principal, session_ref: str, request_key: str):
    ...
    log = TutorLearnerContextLog(public_ref=operation_ref, session_id=session.id, student_id=principal.user.id,
        subject_id=session.subject_id, active_unit_id=session.active_unit_id, active_topic_id=session.active_topic_id, request_key=request_key,
