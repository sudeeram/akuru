...
from app.models import (AssessmentCurriculumSnapshot, AuditEvent, CurriculumPlan, CurriculumPlanTopic,
    OfficialMaterialVersion, OfficialQuestionTopicMapping, OfficialQuestionVersion, StudentProgression,
    StudentSubject, Textbook, TextbookGroup, TextbookTopic, TextbookTopicContentVersion)
from app.schemas.curriculum_plans import (CoverageDiagnosticResponse, CurriculumPlanResponse, PlanChangeResponse,
    PlanPeriod, PlanTopicGroupResponse, PlanTopicResponse, QuestionPoolDiagnosticResponse, SavePlanRequest)
...
def _topic_response(db, plan, book):
    ...
    return CurriculumPlanResponse(versionNumber=plan.version_number if plan else 0, status=plan.status if plan else "not_started",
        subjectId=book.subject_id, textbookTitle=book.title, textbookEdition=book.edition, groupLabel=book.group_label,
...
def get_plan(db: Session, subject_id: str):
    ...
    if not book: raise DomainError("topic_textbook_required", "Publish the logical textbook structure before configuring coverage.", 409)
    return _topic_response(db, _plan(db, subject_id), book)
...
def create_next_draft(db: Session, principal: Principal, subject_id: str):
    ...
    plan = CurriculumPlan(course_id="igcse", subject_id=subject_id, textbook_id=book.id,
...
def snapshot(db: Session, assessment_ref: str, student_id: uuid.UUID, subject_id: str):
    ...
    row = AssessmentCurriculumSnapshot(assessment_ref=assessment_ref, student_id=student_id, plan_id=plan.id, course_id="igcse", subject_id=subject_id, grade=current.grade, term=current.term, covered_topic_ids=topic_ids, progression_periods=[{"grade":r.grade,"term":r.term} for r in progress if (r.grade,r.term)<=(current.grade,current.term)])
