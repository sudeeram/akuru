...
def _eligible_question(db, student_id, subject_id, unit_id):
    coverage = student_coverage(db, student_id, subject_id)
    allowed = {uuid.UUID(row.id) for row in coverage.coveredUnits} if coverage.status == "ready" else set()
    if unit_id not in allowed: return None
    candidates = db.scalars(select(OfficialQuestionVersion).join(OfficialMaterialVersion,
        ...
        OfficialQuestionUnitMapping, OfficialQuestionUnitMapping.question_version_id == OfficialQuestionVersion.id).where(
        ...
        OfficialQuestionVersion.mapping_status == "confirmed", OfficialQuestionUnitMapping.status == "confirmed",
        OfficialQuestionUnitMapping.unit_id == unit_id).order_by(OfficialQuestionVersion.id)).all()
    for question in candidates:
        mapped = set(db.scalars(select(OfficialQuestionUnitMapping.unit_id).where(
...
def list_recommendations(db, principal, student_id=None):
    ...
    query = select(ImprovementRecommendation, WeaknessDiagnosis, TextbookUnit, RetrievalChunk, Document).join(
