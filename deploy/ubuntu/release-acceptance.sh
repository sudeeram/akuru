...
headers="$(curl --fail --silent --show-error --max-time 15 --head "https://${host}/health")"
...
for service in akuru-api akuru-worker akuru-web; do systemctl is-active --quiet "$service"; done
for port in 5432 6379 8000 5181; do
  ! ss -lnt | awk '{print $4}' | grep -Eq "(^0\\.0\\.0\\.0:${port}$|^\\[::\\]:${port}$)"
done
recent_errors="$(journalctl -u akuru-api -u akuru-worker -u akuru-web --since '-10 min' --priority=err --no-pager || true)"
if [[ -n "$recent_errors" ]]; then
  echo 'Recent service errors found; inspect logs before accepting this release.' >&2
  exit 1
fi
echo "AKURU HTTPS, headers, service health and loopback-binding checks passed for ${host}."
