...
Compare it with the marking points. Enter a whole-number mark from zero to the
available marks, add specific feedback and select Save review.

Tell the child what was correct and what to improve. Previous marks/feedback
are retained. Exact numeric checking only checks a final answer; it does not
...
grades or proof that the full syllabus is mastered. Discuss the child's study
plan and repeated difficulties, then assign a small amount of focused practice.

Sign out after using a shared device. Report wrong family links to Admin.

STEP 17: REVIEWING YOUR CHILD'S RESULTS
Open Assessment reviews and choose a child, or view all your linked children.
Each child's summary includes unit scores, confidence and the latest ten score
changes. These are learning indicators, not predicted exam grades.

Open a question to inspect the submitted answer and any uploaded working.
Review each official marking point, its evidence and awarded marks. You can
correct the explanation, improved answer, strengths and mistakes. Enter your
reason and select Publish reviewed result. AKURU retains the previous result
and recalculates mastery. If a newer result exists, refresh before reviewing it.
